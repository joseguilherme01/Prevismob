...
| ------ | ----------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------ |
| CT-001 | Acessar a landing page sem login                | Implementado         | A página inicial carrega com os botões "Entrar" e "Criar conta" visíveis na barra de navegação.  |
| CT-002 | Clicar em "Avaliar Imóvel" sem estar logado     | Implementado         | O sistema redireciona o visitante para a página de previsão sem exigir login.                    |
| CT-003 | Fazer a 1ª previsão como visitante              | Implementado         | O formulário é preenchido e o resultado é exibido normalmente. O contador de cotas marca 1 de 2. |
| CT-004 | Fazer a 2ª previsão como visitante              | Implementado         | A previsão é realizada com sucesso. O contador marca 2 de 2 — cota esgotada.                     |
| CT-005 | Tentar fazer a 3ª previsão como visitante       | Implementado         | O sistema bloqueia a tentativa e exibe uma mensagem sugerindo criar conta para continuar.        |
| CT-006 | Tentar acessar o histórico sem login            | Implementado         | O sistema não exibe dados de histórico para visitantes não autenticados.                         |
| CT-007 | Tentar acessar a página de comparação sem login | Implementado         | A página de comparação exige autenticação; visitantes são redirecionados.                        |

### Grupo 2 — Cadastro (CT-008 a CT-012)

| ID     | Condição Relacionada                               | Status Implementação | Observações                                                                                          |
| ------ | -------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------- |
| CT-008 | Tentar cadastrar com campos em branco              | Implementado         | O sistema exibe mensagens de erro nos campos obrigatórios e não conclui o cadastro.                  |
| CT-009 | Tentar cadastrar com e-mail em formato inválido    | Implementado         | O campo de e-mail exibe erro indicando formato incorreto antes de enviar.                            |
| CT-010 | Digitar senha fraca e verificar o indicador visual | Implementado         | O indicador de força da senha muda conforme o usuário digita, mostrando "Fraca", "Média" ou "Forte". |
| CT-011 | Cadastrar com nome, e-mail e senha válidos         | Implementado         | O cadastro é concluído e o sistema informa que um e-mail de verificação foi enviado.                 |
| CT-012 | Tentar fazer login antes de verificar o e-mail     | Implementado         | O sistema bloqueia o acesso e exibe uma mensagem orientando a confirmar o e-mail.                    |

### Grupo 3 — Login e acesso autenticado (CT-013 a CT-018)

| ID     | Condição Relacionada                       | Status Implementação | Observações                                                                                         |
| ------ | ------------------------------------------ | -------------------- | --------------------------------------------------------------------------------------------------- |
| CT-013 | Tentar fazer login com senha incorreta     | Implementado         | O sistema exibe mensagem de erro e não concede acesso.                                              |
| CT-014 | Fazer login com e-mail e senha corretos    | Implementado         | O login é bem-sucedido, a barra de navegação muda para o estado autenticado e a cota exibe 0 de 10. |
| CT-015 | Fazer login com conta Google               | Implementado         | O botão "Entrar com Google" abre o fluxo de autenticação e o login é concluído com sucesso.         |
| CT-016 | Fazer até 10 previsões estando logado      | Implementado         | Cada previsão é aceita normalmente; o contador de cotas vai de 0 até 10.                            |
| CT-017 | Tentar fazer a 11ª previsão estando logado | Implementado         | O sistema bloqueia a tentativa com uma mensagem informando que o limite diário foi atingido.        |
| CT-018 | Verificar o indicador de cota na tela      | Implementado         | O indicador ("badge") na página de previsão exibe o número de previsões usadas e o limite total.    |

### Grupo 4 — Histórico e comparação (CT-019 a CT-025)

| ID     | Condição Relacionada                            | Status Implementação | Observações                                                                            |
| ------ | ----------------------------------------------- | -------------------- | -------------------------------------------------------------------------------------- |
| CT-019 | Acessar o histórico estando logado              | Implementado         | A página de histórico exibe os cards das avaliações realizadas pelo usuário.           |
| CT-020 | Selecionar 1 avaliação no histórico             | Implementado         | Um card é selecionado e o botão flutuante "Comparar 1 selecionado" aparece na tela.    |
| CT-021 | Selecionar 2 avaliações no histórico            | Implementado         | Dois cards são selecionados e o botão flutuante exibe "Comparar 2 selecionados".       |
| CT-022 | Tentar selecionar uma 3ª avaliação no histórico | Implementado         | O sistema impede a seleção de mais de 2 avaliações simultaneamente.                    |
| CT-023 | Clicar em "Comparar" e verificar a página       | Implementado         | A página de comparação abre exibindo as duas avaliações lado a lado com gráfico radar. |
| CT-024 | Exportar a comparação em CSV                    | Implementado         | O arquivo CSV é gerado e o download é iniciado automaticamente pelo navegador.         |
| CT-025 | Exportar a comparação em PDF                    | Implementado         | O arquivo PDF é gerado e o download é iniciado automaticamente pelo navegador.         |

### Grupo 5 — Navegação (CT-026 a CT-029)

| ID     | Condição Relacionada                        | Status Implementação | Observações                                                                                         |
| ------ | ------------------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------- |
| CT-026 | Usar o botão "Voltar" na página de previsão | Implementado         | O botão "← Voltar" no cabeçalho leva o usuário de volta à landing page.                             |
| CT-027 | Usar o botão de voltar do próprio navegador | Implementado         | O botão de voltar do navegador retorna à landing page em vez de sair do site.                       |
| CT-028 | Clicar em "Histórico" na barra de navegação | Implementado         | O link "Histórico" no cabeçalho navega corretamente para a página de histórico.                     |
| CT-029 | Clicar em "Sair" para encerrar a sessão     | Implementado         | O logout é realizado, a sessão é encerrada e o usuário é redirecionado para a landing page pública. |
...
| **Ferramenta**           | Google Chrome                                                                                                 |
| **Ambiente**             | `http://127.0.0.1:8000`                                                                                       |
| **Critério de execução** | Testes manuais conduzidos diretamente na interface do usuário, seguindo a ordem dos grupos definidos no plano |
